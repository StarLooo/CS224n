...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # the decoder:
        # the input_tensor is ybar_t, h_{t-1} and c_{t-1}. the output is h_{t} and c_{t}
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]
        ...
        print("source_padded:")
        print("shape:", source_padded.shape)
        print("val:", source_padded)
        X = self.model_embeddings.source(source_padded)
        print("X:")
        print("shape:", X.shape)
        print("source_lengths:")
        print("val:", source_lengths)
        ...
        print("last_cell:")
        print("shape:", last_cell.shape)
        enc_hiddens = pad_packed_sequence(enc_hiddens, batch_first=True)[0]
        last_hidden = torch.cat(tensors=(last_hidden[0, :], last_hidden[1, :]), dim=1)
        ...
        print("last_hidden after cat:")
        print("shape:", last_hidden.shape)
        print("shape:", last_cell.shape)
        dec_init_state = (init_decoder_hidden, init_decoder_cell)
        print("enc_hiddens:")
        print("shape:", enc_hiddens.shape)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        print(torch.unsqueeze(dec_hidden, dim=2).shape)
        ...
        # enc_mask makes the probability of <padded> approaching 0
        ...
        # a_t shape: (b, 2h)
        a_t = torch.squeeze(
            torch.bmm(torch.unsqueeze(alpha_t, dim=1), enc_hiddens),
            dim=1)  # context vector
        # U_t shape: (b, 3h)
