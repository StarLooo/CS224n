...
def sigmoid(x):
    """
    Compute the sigmoid function for the input_tensor here.
    Arguments:
...
def naiveSoftmaxLossAndGradient(
    ...
    delta = y_hat.copy()
    delta[outsideWordIdx] -= 1  # delta = y_hat - y
...
def negSamplingLossAndGradient(
    ...
    z = sigmoid(np.dot(outsideVector, centerWordVec))
    loss -= np.log(z)
    gradCenterVec += outsideVector * (z - 1.0)
    gradOutsideVecs[outsideWordIdx] += centerWordVec * (z - 1.0)

    IMPLEMENTATION_METHOD = "loop"
    # Loop Implementation (Slow)
    if IMPLEMENTATION_METHOD == "loop":
        for k in range(1, K + 1):
            sampled_id = indices[k]
            negtiveSampleVector = outsideVectors[sampled_id]
            z = sigmoid(np.dot(-negtiveSampleVector, centerWordVec))
            loss -= np.log(z)
            gradCenterVec -= negtiveSampleVector * (z - 1.0)
            gradOutsideVecs[sampled_id] -= centerWordVec * (z - 1.0)
    # Vectorized Implementation (Recommended, But Still have BUG that not solverd)
    # if IMPLEMENTATION_METHOD == "vectorized":
    #     negSampleVectors = outsideVectors[negSampleWordIndices]
    #     z = sigmoid(-np.dot(negSampleVectors, centerWordVec))
    #     loss -= np.sum(np.log(z))
    #     gradCenterVec += np.dot((1 - z), negSampleVectors)
    #     dim = len(centerWordVec)
    #     A = np.repeat((1 - z), dim).reshape(K, dim)
    #     B = np.array([centerWordVec.tolist()] * K)
    #     # TODO: STILL HAVE BUG TO SOLVE!
    #     gradOutsideVecs[negSampleWordIndices] += A * B
    # END YOUR CODE
