...
class ModelEmbeddings(nn.Module):
    """
    Class that converts input_tensor words to their CNN-based embeddings.
    """
    ...
    def forward(self, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        X_word_emb_list = []
        # divide input_tensor into sentence_length batchs
        for X_padded in input_tensor:
            X_emb = self.char_embedding(X_padded)  # shape: (batch_size, sentence_length, e_char)
            X_reshaped = torch.transpose(X_emb, dim0=-1, dim1=-2)
            # conv1d can only take 3-dim mat as input_tensor
        ...
        X_word_emb = torch.stack(X_word_emb_list)  # shape: (sentence_length, batch_size, e_word)
        # print("X_word_emb_list len:", len(X_word_emb_list))
        # print("X_word_emb shape:", X_word_emb.shape)
        # os.system("pause")
        return X_word_emb
