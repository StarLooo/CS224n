...
class NMT(nn.Module):
    ...
    def __init__(self,
        ...
        @param char_embed_size (int): Embedding size (dimensionality)
        ...
        self.combined_output_projection = nn.Linear(in_features=hidden_size * 2 + hidden_size, out_features=hidden_size,
                                                    bias=False)
        ...
        if not no_char_decoder:
            self.charDecoder = CharDecoder(hidden_size=hidden_size, target_vocab=vocab.tgt)
        else:
    ...
    def forward(self, source: List[List[str]],
        ...
        @returns scores (Tensor): a variable/tensor of shape (b, ) representing the
                                    ...
                                    each example in the input_tensor batch. Here b = batch size.
        """
        ...
        target_padded = self.vocab.tgt.to_input_tensor(
            target, device=self.device)  # Tensor: (src_len, batch_size)

        source_padded_chars = self.vocab.src.to_input_tensor_char(
            source, device=self.device)  # Tensor: (src_len, batch_size, m_word_len)
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(
            target, device=self.device)  # Tensor: (src_len, batch_size, m_word_len)

        enc_hiddens, dec_init_state = self.encode(source_padded=source_padded_chars, source_lengths=source_lengths)
        enc_masks = self.generate_sent_masks(enc_hiddens, source_lengths)
        combined_outputs = self.decode(enc_hiddens=enc_hiddens, enc_masks=enc_masks, dec_init_state=dec_init_state,
                                       target_padded=target_padded_chars)
        ...
        target_gold_words_log_prob = torch.gather(P, index=target_padded[1:].unsqueeze(dim=-1), dim=-1).squeeze(
            -1) * target_masks[1:]
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]
        ...
        # all rnn receive PackedSequence as input_tensor!!!
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        @param Ybar_t (Tensor): Concatenated Tensor of [Y_t o_prev], with shape (b, e + h). The input_tensor for the decoder,
    ...
    def beam_search(self,
        ...
        while len(completed_hypotheses
            ...
            for prev_hyp_id, hyp_word_id, cand_new_hyp_score in zip(
                ...
                # print("prev_hyp_id:", prev_hyp_id)
                # print("hyp_word:", hyp_word)
                # os.system("pause")
