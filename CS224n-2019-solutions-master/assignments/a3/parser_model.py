...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        @param n_features (int): number of input_tensor features
        @param hidden_size (int): number of hidden units
        @param n_classes (int): number of output classes
        @param dropout_prob (float): dropout probability
        """
        super(ParserModel, self).__init__()
        self.num_words, self.embed_size = embeddings.shape
        self.n_features = n_features
        self.hidden_size = hidden_size
        self.n_classes = n_classes
        self.dropout_prob = dropout_prob
        self.pretrained_embeddings = nn.Embedding(self.num_words, self.embed_size)
    ...
    def embedding_lookup(self, t):
        """ Utilize `self.pretrained_embeddings` to map input_tensor `t` from input_tensor tokens (integers)
            to embedding vectors.

            PyTorch Notes:
                - `self.pretrained_embeddings` is a torch.nn.Embedding object that we defined in __init__
                - Here `t` is a tensor where each row represents a list of features. Each feature is represented
                    by an integer (input_tensor token).
            ...
            @param t : input_tensor tensor of tokens (batch_size, n_features) (Tensor)

            @return x : tensor of embeddings for words represented in t
                                (batch_size, n_features * char_embed_size) (Tensor)
        """
        # YOUR CODE HERE (~1-3 Lines)
        #     1) Use `self.pretrained_embeddings` to lookup the embeddings for the input_tensor tokens in `t`.
    ...
    def forward(self, t):
        """ Run the model forward.
            ...
            Note that we will not apply the softmax function here because it is included in
            the loss function nn.CrossEntropyLoss

            PyTorch Notes:
                - Every nn.Module object (PyTorch model) has a `forward` function.
                - When you apply your nn.Module to an input_tensor tensor `t` this function is applied to the tensor.
        ...
        @return logits : tensor of predictions (output after applying the layers of the network)
                                 without applying softmax (batch_size, n_classes) (Tensor)
        """
